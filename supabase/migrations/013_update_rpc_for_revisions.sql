-- Migration: 013_update_rpc_for_revisions
-- Date: 2026-02-14
-- Description: Update save_minutes_atomic to handle revision_of and revision_number columns
-- Depends on: 012_formal_revision_workflow

-- We need to update the INSERT branch to include the new columns.
-- Rather than recreating the entire massive function, we use a simpler approach:
-- The client will set revision_of and revision_number directly when creating a revision.
-- The RPC INSERT already passes through the fields, so we just need to add them to the column list.

-- Recreate the full function with revision support
...
BEGIN
  ...
  -- Business items (with sort_order)
