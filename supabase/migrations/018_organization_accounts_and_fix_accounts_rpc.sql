-- Migration: 018_organization_accounts_and_fix_accounts_rpc
-- Date: 2026-02-27
-- Description: 
--   1. Create organization_accounts table for predefined financial accounts
--   2. Add RLS policies (same pattern as subcommittees)
--   3. FIX BUG-901: Add missing 'accounts' JSONB field to save_minutes_atomic INSERT and UPDATE
-- Depends on: 017_email_verification_in_rpcs

-- ============================================================
-- PART 1: Organization Accounts table
-- ============================================================
...
CREATE INDEX idx_org_accounts_org_id ON organization_accounts(organization_id);

-- Enable RLS
ALTER TABLE organization_accounts ENABLE ROW LEVEL SECURITY;
ALTER TABLE organization_accounts FORCE ROW LEVEL SECURITY;

-- RLS: Users can view accounts for their own org (admin + editor + viewer)
...
-- RLS: Admin and Editor can insert accounts for their own org
...
-- RLS: Admin and Editor can update accounts for their own org
...
-- RLS: Admin can delete accounts for their own org
...
-- ============================================================
-- PART 2: FIX BUG-901 — Add 'accounts' to save_minutes_atomic
-- ============================================================
-- The 'accounts' column (JSONB, added in migration 008) was never included
-- in the INSERT or UPDATE within this RPC. Financial data entered by
-- secretaries was silently dropped on every save.
...
BEGIN
  ...
  -- CR-001 FIX: Require verified email (matches RLS restrictive policies)
  ...
  IF v_minutes_id IS NULL THEN
    INSERT INTO minutes (
      ...
      accounts,  -- BUG-901 FIX: was missing
    ...
    VALUES (
      ...
      COALESCE(p_minutes->'accounts', '[]'::jsonb),  -- BUG-901 FIX
  ...
  ELSE
    IF v_loaded_at IS NOT NULL THEN
      ...
      SET
        ...
        accounts                     = COALESCE(p_minutes->'accounts', '[]'::jsonb),  -- BUG-901 FIX
    ...
    ELSE
      ...
      IF v_minutes_id IS NULL THEN
        RAISE EXCEPTION 'Minutes not found or access denied'
          USING ERRCODE = '42501';
      END IF;
    END IF;
  END IF;
  ...
  SELECT v_minutes_id,
         elem->>'item_type',
         elem->>'title',
         NULLIF(elem->>'discussion',''),
         elem->'motion',
         COALESCE((elem->>'no_motion')::boolean, true),
         ROW_NUMBER() OVER ()
  ...
  SELECT v_minutes_id,
         elem->>'task',
         NULLIF(elem->>'assignee_name',''),
         NULLIF(elem->>'assignee_id','')::uuid,
         NULLIF(elem->>'due_date','')::date,
