...
export function useMinutes() {
  ...
  const fetchFullMinutes = useCallback(async (id) => {
    ...
    // BUG-305 FIX: Throw on any child table error instead of returning partial data
    if (mErr) { console.error('Error fetching minutes:', mErr); throw mErr; }
    if (!mins) return null;
    const childErrors = [aErr, bErr, acErr, fErr, uErr].filter(Boolean);
    if (childErrors.length > 0) {
      console.error('Error fetching minutes child data:', childErrors);
      throw new Error(`Failed to load complete minutes data: ${childErrors.map(e => e.message).join('; ')}`);
    }

    return {
      ...mins,
      _loaded_at: mins.updated_at, // BUG-008 FIX: Use DB timestamp, not client clock
      attendance: att || [],
      businessItems: biz || [],
      actionItems: actions || [],
  ...
  const saveMinutes = useCallback(async (minutesData) => {
    ...
    const p_financial_items = financialItems.map(f => ({
      ...
      amount: stripMoney(f.amount) || '0', // BUG-009 FIX: Strip $, commas from line items
    }));
