...
const [deleteScTarget, setDeleteScTarget] = useState(null);
  ...
  const handleSave = async () => {
    ...
    try {
      // BUG-703 FIX: Validate hex colors before saving to prevent CSS injection at source
      const isValidHex = (c) => /^#[0-9a-fA-F]{3,8}$/.test(c);
      const safePrimary = isValidHex(form.primary_color) ? form.primary_color : '#1e3a5f';
      const safeAccent = isValidHex(form.accent_color) ? form.accent_color : '#dc2626';
      ...
      const { error: updateError } = await supabase.from('organizations').update({
        ...
        primary_color: safePrimary, accent_color: safeAccent,
  ...
  const saveSC = async () => {
    ...
    try {
      ...
      // Sync members
      const { error: delErr } = await supabase.from('subcommittee_members').delete().eq('subcommittee_id', scId);
      ...
      if (memberIds.length > 0) {
        ...
        const { error: insErr } = await supabase.from('subcommittee_members').insert(rows);
