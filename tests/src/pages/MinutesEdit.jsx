...
const { toast } = useToast();
  ...
  // BUG-304 FIX: Also block in-app React Router navigation when dirty
  ...
  useEffect(() => {
    if (isNew) {
      ...
      fetchFullMinutes(id).then(data => {
        ...
        toast.error('Failed to load minutes: ' + (err.message || 'Unknown error'));
        navigate('/minutes');
      });
    }
...
<ConfirmDialog
      ...
      <SendEmailModal
        ...
        members={members}
      />
      {/* BUG-304 FIX: Block in-app navigation when there are unsaved changes */}
