...
const { toast } = useToast();
  ...
  // BUG-3003: Warn on unsaved changes when navigating away
...
const isNew = !id;
  ...
  const handleSave = async (status) => {
    ...
    try {
      ...
      // BUG-3002: Update _loaded_at so subsequent saves use optimistic locking
      setDraft(prev => ({ ...prev, id: result.id, _loaded_at: result.updated_at }));
      setIsDirty(false);
      if (isNew) navigate('/agendas/' + result.id, { replace: true });
...
<ConfirmDialog
      ...
      {/* BUG-3003: Unsaved changes warning */}
      {blocker.state === 'blocked' && (
        <ConfirmDialog
          ...
          message="You have unsaved changes to this agenda. Are you sure you want to leave?"
          confirmLabel="Leave"
          variant="danger"
        />
      )}
